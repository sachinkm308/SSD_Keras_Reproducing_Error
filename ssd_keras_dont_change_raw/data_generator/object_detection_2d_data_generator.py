...
class DataGenerator:
    ...
    def generate(self,
        ...
        while True:
            ...
            #print('printing first batch_X', batch_X.shape)
                    ...
                    for transform in transformations:
                        ...
                        if not (self.labels is None):
                            ...
                            if ('inverse_transform' in returns) and ('return_inverter' in inspect.signature(transform).parameters):
                                ...
                                print('printing 5th batch_X', np.asarray(batch_X).shape)
                                inverse_transforms.append(inverse_transform)
                            else:
                                batch_X[i], batch_y[i] = transform(batch_X[i], batch_y[i])
                                print('printing 6th batch_X', np.asarray(batch_X).shape)
                            ...
                            if batch_X[i] is None: # In case the transform failed to produce an output image, which is possible for some random transforms.
                                batch_items_to_remove.append(i)
                                batch_inverse_transforms.append([])
                                print('printing 7th batch_X', np.asarray(batch_X).shape)
                                continue
                            ...
                            if ('inverse_transform' in returns) and ('return_inverter' in inspect.signature(transform).parameters):
                                ...
                                inverse_transforms.append(inverse_transform)
                                print('printing 8th batch_X', np.asarray(batch_X).shape)
                            else:
                                batch_X[i] = transform(batch_X[i])
                                print('printing 9th batch_X', np.asarray(batch_X).shape)
            ...
            if batch_items_to_remove:
                for j in sorted(batch_items_to_remove, reverse=True):
                    ...
                    batch_X.pop(j)
                    print('printing 10th batch_X', np.asarray(batch_X).shape)
            ...
            #print("printing batch_X original value - unchanged version -->", batch_X.shape)
            if (batch_X.size == 0):
